...
export default function queryMiniLyrics(
  title: string,
  artist: string,
  cb: (error: Error | undefined, data?: MinilyricsResponse) => void
) {
  // Send request
  ...
  const query =
    ...
    `<searchV1 filetype="lyrics" ClientCharEncoding="utf-8" artist="${
      artist || ''
    }" title="${title || ''}" client="MiniLyrics" />`;
...
function encryptString(query: string) {
  ...
  let data = [0x02, key, 0x04, 0x00, 0x00, 0x00, ...md5Bytes, ...queryBytes];
  return data;
}
...
class CompressedXmlReader {
  ...
  private readSlice(byteCount?: number) {
    if (byteCount === undefined)
      byteCount = this.buffer.length - this.position + 1;
  ...
  private readHeader() {
    ...
    if (this.readInt() !== 2) throw new Error('Header version mismatch');
  ...
  private readStringTable() {
    ...
    if (this.stringTable.length !== stCount) {
      console.warn(
        `String table should contain ${stCount} items, but has ${this.stringTable.length}`
      );
    }
  }
